...
if ( !function_exists( 'znb_get_svg' ) )
{
	function znb_get_svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'zion-builder' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'zion-builder' );
		}
...
function znb_background_source( $args = array() )
{
	...
	if($args['mobile_play'] == 'yes'){
		...
		if(!empty($mp_video)){
			echo '<a href="'.$mp_video.'" class="mfp-iframe zn-bgSource-videoModal visible-xs visible-sm" data-text="'. __('PLAY VIDEO', 'zion-builder') .'">'. znb_get_svg(array('icon'=>'znb_play')) .'</a>';
		}
	}
...
if ( !function_exists( 'znb_animation_helper' ) ){
	function znb_animation_helper( $options )
	{
		...
		$default_options = array(
			array(
				'name'        => __('Appear animation', 'zion-builder'),
				'description' => __('Select the appear animation for this element when it comes into the viewport', 'zion-builder'),
				'id'          => 'appear_animation',
				'type'        => 'select',
				'std'         => 'none',
				'options'     => array(
			...
			array(
				'name'        => __('Animation Duration', 'zion-builder'),
				'description' => __('Select the type of duration.', 'zion-builder'),
				'id'          => 'appear_duration',
				'std'         => '1000',
				'type'        => 'select',
				'options'     => array(
					'500'  => __( "Fast (500ms)", 'zion-builder' ),
					'1000' => __( "Normal (1000ms)", 'zion-builder' ),
					'2000' => __( "Slow (2000ms)", 'zion-builder' ),
				),
			),
			array(
				'name'        => __('Animation Delay', 'zion-builder'),
				'description' => __('Enter the number of milliseconds the animation is delayed after the element comes into the viewport. Please note that some elements can contain multiple elements that will animate progressively based on this value.', 'zion-builder'),
				'id'          => 'appear_delay',
				'type' => 'slider',
				'std' => '700',
				'helpers' => array(
...
function zn_margin_padding_options( $uid = '', $args = array() ){
	...
	if( $args['responsive'] ){
		$options[] = array (
			"name"        => __( "Edit element padding & margins for each device breakpoint. ", 'zion-builder' ),
			"description" => __( "This will enable you to have more control over the padding of the element on each device. Click to see <a href='http://hogash.d.pr/1f0nW' target='_blank'>how box-model works</a>.", 'zion-builder' ),
	...
	if( $args['margin'] ){
		...
		$options[] = array(
			...
			'description' => __('Select the margin (in percent % or px) for this element. Accepts negative margin.', 'zion-builder'),
			'type'        => 'boxmodel',
		...
		if( $args['responsive'] ){
			$options[] = array(
				...
				'description' => __('Select the margin (in percent % or px) for this element.', 'zion-builder'),
				'type'        => 'boxmodel',
	...
	if( $args['padding'] ){
		...
		$options[] = array(
			...
			'description' => __('Select the padding (in percent % or px) for this element.', 'zion-builder'),
			'type'        => 'boxmodel',
