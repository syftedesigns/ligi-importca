<?php if ( !defined( 'ABSPATH' ) ) {
	return;
}

/**
 * @TODO : Trebuie sa ascundem smart area-urile cand editezi pagina cu PB
 */

/**
 * Class ZionSmartArea
 *
 * Registers the custom post type "znpb_template_mngr" that will be used to create Smart Areas
 */
class ZionSmartArea
{
	/**
	 * Holds the list of all registered smart areas
	 * @var array
	 */
	private $registered_smart_areas = array();

	/**
	 * Class constructor
	 */
	function __construct()
	{
		add_action( 'init', array( $this, 'registerPostType' ), 99 );
		add_action( 'admin_notices', array( $this, 'displaySmartAreaInfo' ) );
	}


	/**
	 * Register a smart area id that is loaded on the current page
	 *
	 * @param  int $smart_area_id The smart area id that we need to register
	 */
	function registerSmartArea( $smart_area_id )
	{
		$this->registered_smart_areas[ $smart_area_id ] = $smart_area_id;
	}

	/**
	 * Retrieve all registered Smart Areas
	 * @return array
	 */
	function getRegisteredSmartAreas()
	{
		return $this->registered_smart_areas;
	}

	/**
	 * Register the custom post type
	 */
	function registerPostType()
	{
		$labels = array(
			'name'               => __( 'Page Builder Smart Areas', 'zion-builder' ),
			'singular_name'      => __( 'Page Builder Smart Area', 'zion-builder' ),
			'add_new'            => __( 'Add New', 'zion-builder' ),
			'add_new_item'       => __( 'Add New Page Builder Smart Area', 'zion-builder' ),
			'edit_item'          => __( 'Edit Page Builder Smart Area', 'zion-builder' ),
			'new_item'           => __( 'New Page Builder Smart Area', 'zion-builder' ),
			'view_item'          => __( 'View Page Builder Smart Area', 'zion-builder' ),
			'search_items'       => __( 'Search Page Builder Smart Areas', 'zion-builder' ),
			'not_found'          => __( 'No Page Builder Smart Areas found', 'zion-builder' ),
			'not_found_in_trash' => __( 'No Page Builder Smart Areas found in Trash', 'zion-builder' ),
			'parent_item_colon'  => '',
		);

		$icon = "data:image/svg+xml;base64,";

		$args = array(
			'labels'             => $labels,
			'menu_icon'          => $icon,
			'public'             => true,
			'publicly_queryable' => true,
			'exclude_from_search'=> true,
			'show_in_nav_menus'  => false,
			'show_ui'            => true,
			'query_var'          => true,
			'capability_type'    => 'post',
			'hierarchical'       => false,
			'menu_position'      => 106,
			'supports'           => array( 'title' ),
			'show_in_menu'       => true,
		);

		register_post_type( 'znpb_template_mngr', $args );
	}


	/**
	 * Display the Smart Area notice in the backend as a dismissible admin notice
	 */
	function displaySmartAreaInfo()
	{
		$screen = get_current_screen();
		if ( $screen->id == 'edit-znpb_template_mngr' ) {
			?>
			<div class="notice notice-info is-dismissible">
				<p>
					<?php
					echo sprintf( __( 'Page builder Smart Areas are predefined blocks of page builder elements you can insert in your website\'s pages, globally or per page. <a href="%s" target="_blank">Read more</a>.', 'zion-builder' ), esc_url( 'http://support.hogash.com/documentation/page-builder-smart-areas/' ) );
					?>
				</p>
			</div>
			<?php
		}
	}

}
