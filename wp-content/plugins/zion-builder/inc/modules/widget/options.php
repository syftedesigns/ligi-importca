...
if(isset($widget_slug) && isset($wp_widget_factory->widgets[$widget_slug])) {
	...
	// Widget title
	echo '<h3 class="zn-pb-widget-options">' . $widget_instance->name . ' '.__( 'Widget Options', 'zion-builder' ).'</h3>';
	...
	printf( _x( '%s does not exists.', '%s stands for widget slug.', 'zion-builder' ), $widget_slug );
}
else{
	_e('There was a problem getting the widget options', 'zion-builder');
}
