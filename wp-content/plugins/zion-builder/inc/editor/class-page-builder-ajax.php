...
function znpb_get_module_option() {
	...
	if ( ! empty( $selector ) ) {
		echo '<div title="' . esc_html(__('You can use this css selector to customize this element', 'zion-builder')) . '" class="zn_unique_selector_bar">' . __('CSS selector:', 'zion-builder') . ' ' . $selector . '</div>';
	}
...
function znpb_save_module() {
	...
	$option = array (
		'name'        => __( 'Element name', 'zion-builder'),
		'description' => __( 'Enter a name for this saved element. Please note that the name must be unique!', 'zion-builder'),
		'id'          => 'element_name',
		'std'         => '',
		'type'        => 'text',
		'placeholder' => __( 'My awesome element', 'zion-builder' ),
		'class'		     => 'zn_full',
	);
	...
	if ( empty( $_POST['element_uid'] ) || empty( $_POST['element_level'] ) ) {
		echo __('Something went wrong. Please try again', 'zion-builder');
	} else {
		echo ZNHGFW()->getComponent('html')->zn_render_single_option( $option );

		// Save button
		echo '<a href="#" data-uid="' . $_POST['element_uid'] . '" data-level="' . $_POST['element_level'] . '" class="zn-btn-confirm zn-btn-green zn_button_save_element">' . __( 'Save element', 'zion-builder' ) . '</a>';
		echo '<a href="#" data-uid="' . $_POST['element_uid'] . '" data-level="' . $_POST['element_level'] . '" class="zn-btn-confirm zn-btn-cancel zn_button_export_element">' . __( 'Export element', 'zion-builder' ) . '</a>';
	}
...
function znpb_do_save_element( $template_data = false ) {
	...
	if ( $result ) {
		$return['message'] = __('Element successfully saved.', 'zion-builder');
	} else {
		$return['message'] = __('There was a problem saving the element.', 'zion-builder');
	}
...
function zn_save_template() {
	...
	$result = update_post_meta( $post_id, $template_new_name, $content );
	if ( $result ) {
		$return['message'] = __('Template successfully saved.', 'zion-builder');
	} else {
		$return['message'] = __('There was a problem saving the template.', 'zion-builder');
	}
...
function zn_delete_template() {
	...
	if ( $template_deleted ) {
		$return['message'] = __('Template deleted successfully', 'zion-builder');
		$return['success'] = true;
	} else {
		$return['message'] = __('There was a problem. The template was not deleted', 'zion-builder');
	}
