	...
	function renderUneditableContent( $elements, $area_id = false ) {
		...
		if ( $pbstate ) {
			...
			if ( $area_id ) {
				...
				$html .= '<a class="znhg-uneditable-area-url" target="_blank" href="' . $edit_url . '">' . __( 'Edit smart area with pagebuilder', 'zion-builder' ) . '<span class="dashicons dashicons-admin-generic"></span></a>';
			}
		}
	...
	function adminBarMenu( $wp_admin_bar ) {
		...
		if ( ZNB()->utility->isActiveEditor() ) {
			$args = array(
				...
				'title' => __( 'View page', 'zion-builder' ),
			...
			$args = array(
				'id'    => 'znb_edit_button',
				'title' => __( 'Edit with page builder', 'zion-builder' ),
