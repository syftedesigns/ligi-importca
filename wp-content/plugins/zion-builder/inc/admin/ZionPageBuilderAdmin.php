...
class ZionPageBuilderAdmin
{
	...
	function enableEditorAjax()
	{
		...
		if( ! ZNB()->utility->allowedEdit() ){
			wp_send_json_error( array( 'message' => __( 'You don\'t have permission to edit this page!', 'zion-builder' ) ) );
		}
	...
	public function renderRowAction( $actions ){
		$post = get_post();
		$actions['zn_edit'] = '<a href="' . ZNB()->utility->getEditUrl( $post->ID ) . '">' . __( 'Edit with Zion builder', 'zion-builder' ) . '</a>';
		return $actions;
	}
	...
	public static function get_buttons_html(){
		...
		$enable_text 	= __( 'Enable Zion builder', 'zion-builder' );
		$disable_text  	= __( 'Disable Zion builder', 'zion-builder' );
