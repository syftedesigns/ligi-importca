...
class ZnHgFw_Html_Upload extends ZnHgFw_BaseFieldType{
	...
	public function remove_icons() {
		...
		if( ! $nonce_verification ){
			wp_send_json_error(
				array(
					'message' => esc_html( __( 'Security check not passed!', 'hg-framework' ) )
				)
			);
		}
		...
		if ( ! isset( $_POST[ 'font_name' ] ) ) {
			wp_send_json_error( array( 'message' => esc_html( __( 'Invalid request. Missing font name', 'hg-framework' ) ) ) );
		}
	...
	public function upload_icon_package(){
		...
		if ( ! isset( $_POST[ 'attachment' ] ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html( __( 'Invalid request. Missing attachment', 'hg-framework' ) )
				)
			);
		}
		...
		wp_send_json_success( array(
			'message' => sprintf( __( ' The %s font was successfully added', 'hg-framework' ), $icon_install ),
