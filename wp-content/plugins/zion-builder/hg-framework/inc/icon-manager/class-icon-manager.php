...
class ZnHgFw_IconManager {
	...
	*/
	public function do_icons_archive( $zip ) {
		...
		if ( ! $tempdir ) {
			return new WP_Error( 'missing_font_folder', __( 'The temp folder could not be created!', 'hg-framework' ) );
		}
		...
		if ( $extracted ) {
			...
			return new WP_Error( 'missing_font_folder', __( 'The zip file could not be extracted!', 'hg-framework' ) );
		}
	...
	public function create_data() {
		...
		if ( empty( $svg_file ) ) {
			...
			return new WP_Error( 'missing_font_folder', __( 'The zip did not contained any svg files.', 'hg-framework' ) );
		}
		//#! since v4.1.4
		$svgFile = trailingslashit( $this->paths[ 'tempdir' ] ) . $svg_file;
		if ( !is_file( $svgFile ) || !is_readable( $svgFile ) ) {
			zn_delete_folder( $this->paths[ 'tempdir' ] );
			return new WP_Error( 'missing_font_folder', __( 'Could not find or read the svg file.', 'hg-framework' ) );
		}
		...
		if ( !is_wp_error( $file_data ) && !empty( $file_data ) ) {
			...
			if ( ! empty( $icon_list ) && ! empty( $this->font_name ) ) {
				...
				if ( $icon_list_file ) {
					...
					zn_delete_folder( $this->paths[ 'tempdir' ] );
					return new WP_Error( 'missing_font_folder', __( 'There was a problem creating the icon list file', 'hg-framework' ) );
				}
		...
		return new WP_Error( 'missing_font_folder', __( 'The svg file could not be opened.', 'hg-framework' ) );
	...
	*/
	...
	public function delete_font( $font_name ){
		...
		if ( ! is_dir( $dirPath ) ) {
			return new WP_Error( 'missing_font_folder', __( 'Invalid request. Directory not found', 'hg-framework' ) );
		}
	...
	public function create_font_archive( $font_name ){
		...
		if( ! class_exists( 'ZipArchive' ) ) {
			return new WP_Error( 'zip_archive_not_installed', __( 'ZipArchive not installed. Contact your hosting provider and ask them to enable Zip Archive PHP extension.', 'hg-framework' ) );
		}
