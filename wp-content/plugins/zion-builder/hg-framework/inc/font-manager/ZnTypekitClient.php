...
class ZnTypekitClient
{
	...
	public function get( $kitID = null ) {
		if( empty($this->_token)){
			return new WP_Error( __( 'No token provided.', 'hg-framework') );
		}
		$args = array(
			'headers' => array(
		...
		if( empty($body)){
			return new WP_Error( __( 'No data retrieved.', 'hg-framework') );
		}
		$body = json_decode( $body, true );
		if( isset($body['errors'])){
			$errorMessage = sprintf( __( 'No data retrieved: %s', 'hg-framework'), implode(', ', $body['errors']) );
