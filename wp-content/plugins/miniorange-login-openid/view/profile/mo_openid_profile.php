...
function mo_openid_profile(){
    ...
    else if(!mo_openid_is_customer_registered())
    {
        ...
        <div class="mo_openid_table_layout">
            ...
            <br/><label style="cursor: auto"><a href="<?php echo get_site_url() . "/wp-admin/admin.php?page=mo_openid_settings&tab=licensing_plans";?>">Click here</a> to check our <a style="left: 1%; position: static; text-decoration: none" class="mo-openid-premium" href="<?php echo add_query_arg( array('tab' => 'licensing_plans'), $_SERVER['REQUEST_URI'] ); ?>">PRO</a> plans</label>
        </div>
        <?php
    }
    ?>
    <script>
        //to set heading name
        jQuery('#mo_openid_page_heading').text('User Profile Details');
